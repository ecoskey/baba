...
object EventHandler {
	...
	object Mod {
		...
		def apply[e <: Event](
			...
			val handler         : (E, String) => Unit = _handler
			val priority        : EventPriority        = _priority
			val receiveCancelled: Boolean              = _receiveCancelled
		}

		def unapply(handler: EventHandler.Mod): Option[(Class[handler.E], (handler.E, String) => Unit, EventPriority, Boolean)] =
	...
	object Forge {
		...
		def unapply(handler: EventHandler.Forge): Option[(Class[handler.E], (handler.E, String) => Unit, EventPriority, Boolean)] =
			Some(handler.filter, handler.handler, handler.priority, handler.receiveCancelled)
	}
	...
	object GenericMod {
		...
		def apply[e[x] <: GenericEvent[x], a](
			...
			val handler         : (E[A], String) => Unit = _handler
			val priority        : EventPriority          = _priority
			val receiveCancelled: Boolean                = _receiveCancelled
		}
