...
final case class DecMod[R <: IForgeRegistryEntry[R], +O] private(run: Eval[R] => (Chain[CbMod.EventListener], O)) extends AnyVal
...
object DecMod {
	...
	implicit def monadInstance[R <: IForgeRegistryEntry[R]]: Monad[({type T[+O] = DecMod[R, O]})#T] = new Monad[({type T[+O] = DecMod[R, O]})#T] {
		override def pure[A](x: A): DecMod[R, A] = DecMod((_: Eval[R]) => (Chain.nil, x))

		override def flatMap[A, B](fa: DecMod[R, A])(f: A => DecMod[R, B]): DecMod[R, B] = DecMod((r: Eval[R]) => {
	...
	def listener[R <: IForgeRegistryEntry[R], A](f: Eval[R] => CbMod.EventListener): DecMod[R, Unit] = new DecMod(r => (Chain.one(f(r)), ()))
