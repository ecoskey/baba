...
abstract class BabaRecipeBuilderRecipe[B <: RecipeBuilder](
	val internal: B,
	val act     : State[B, _],
	val id      : Option[String],
) extends BabaRecipe {
	private[baba] def save(consumer: Consumer[FinishedRecipe], mod: BaseMod): Unit = id match {
...
object BabaRecipeBuilderRecipe {
	trait Ops[B <: RecipeBuilder] {
		...
		def unlockedByItem(item: Item): State[B, Unit] =
			unlockedBy(
				...
				BabaRecipe.inventoryTrigger(ItemPredicate.Builder.item.of(item).build)
			)
